...
router.post('/', async(req, res ) => {
    try{
      ...
      res.status(401).render('signin', { title: 'Login: Netflix', Link1: '', Link2:'' , error: 'Server error'});
    }
  });
...
router.get('/', (req, res ) => {
    try{
        const {err} = req.query
        return res.status(200).render('signin', { title: 'Login: Netflix', Link1: '', Link2:'' ,loggedIn:false, error:err });
      }
      catch(err){
        console.error(err)
        res.status(200).render('signin', { title: 'Login: Netflix', Link1: '', Link2:'' ,loggedIn:false , error:"server error"});
      }
    })
