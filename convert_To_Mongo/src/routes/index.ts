...
router.get("/", async (req: JwtPayload, res) => {
  try {
    ...
    axios.get(`${HOST}/movies/noOfMovies`, config).then(countMoviesRes => {
      const { noOfMovies, noOfPages } = countMoviesRes.data.data;

      const pageNum = req.query.pageNum;

      axios
        .get(`${HOST}/movies?pageNum=${pageNum || "0"}`, config)
        .then(apiRes => {
  ...
  try {
    ...
    res
      ...
      .render("signin", {
        ...
        Link1: "",
        Link2: "",
        signupError: "User already exist. Check your credentials.",
      });
  }
});
...
router.get("/getallusers", (req, res) => {
  ...
  axios
    .get(`${HOST}/users`, config)
    .then(async apiRes => {
      let result = apiRes.data;
      res
        .status(200)
        .render("getall", {
          title: `All Users : Netflix`,
          Link1: "Logout ",
          Link2: "",
          result,
          loggedIn: true,
...
router.post("/deletemovie", (req, res) => {
  try {
    ...
    axios
      .delete(`${HOST}/movies/${req.body.id}`, config)
      .then(apiRes => {
...
router.post("/deleteuser", (req, res) => {
  try {
    ...
    axios
      .delete(`${HOST}/users/${idEmail}`, config)
      .then(apiRes => {
...
router.post("/updateuser", (req, res) => {
  ...
  if (id) {
    ...
    axios
      .put(`${HOST}/users`, { email, password, fullname, username }, config)
      .then(apiRes => {
...
router.post("/updatemovies", (req, res) => {
  ...
  axios
    .put(`${HOST}/movies`, { title, description, image, price, id: 0 }, config)
    .then(apiRes => {
      res.redirect("/dashboard");
    })
    .catch(err => {
...
router.post("/addmovie", (req: JwtPayload, res) => {
  try {
    ...
    axios.post(`${HOST}/movies`, req.body, config).then(apiRes => {
      res.redirect("/dashboard");
    });
  } catch (error) {
    res.redirect("/dashboard");
